...
import { Cordenadas } from "../components/dash/Map";
...
export async function dataAnalitic(stratum: number) {
    ...
    habitsFamily(data), welfareFamily(data), disabilityPersons(data),getLocations(data)
    ]
}
...
export function getLocations(data: any[]): [number, number][] {
    const coordinates: [number, number][] = data.map(item => [
...
export function getDiccionary(typeNumber: number,diccionary:unknown): string {
  ...
  export function getPromedio(packCoordenadas: any[]): [number, number] {
    let sumaLongitudes = 0;
    let sumaLatitudes = 0;
    console.log(packCoordenadas[i][0])
    for (let i = 0; i < packCoordenadas.length; i++) {
  
      sumaLatitudes += parseFloat(packCoordenadas[i].latitud);
      sumaLongitudes += parseFloat(packCoordenadas[i].longitud);
    }
  
    const promedioLatitud = sumaLatitudes / packCoordenadas.length;
    const promedioLongitud = sumaLongitudes / packCoordenadas.length;
